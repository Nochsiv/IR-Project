...
#         if like_button and not st.session_state.like_button:
#             if option == "BERT":
...
#         # Check if the button has not been clicked before
#         if dislike_button and not st.session_state.dislike_button:
#             if option == "BERT":
...
def sidebar():
    with st.sidebar:
        st.markdown('<style>.rate {font-weight: bold;font-size:20px;color:red}</style>', unsafe_allow_html=True)
        st.markdown(
            f'<p>Model : <span class="rate">BERT</span> => Number of Satisfaction:<span class="rate"> {df.iloc[0]["num_of_rating"]}</span></p>',
            unsafe_allow_html=True)
        st.markdown(
            f'<p>Model : <span class="rate">Okapi BM25</span> => Number of Satisfaction:<span class="rate"> {df.iloc[1]["num_of_rating"]}</span></p>',
            unsafe_allow_html=True)
...
def main():
    ...
    if not duration == 0:
        st.markdown('<style>.duration {font-weight: bold}</style>', unsafe_allow_html=True)
        st.markdown(
            f'<p>Results after <span class="duration">{round((duration / 1000), 3)}</span> seconds</p>',
            unsafe_allow_html=True)
    col1, col2, col3 = st.columns([4, 1, 1])
    ...
    # Update the stored search query in session state
